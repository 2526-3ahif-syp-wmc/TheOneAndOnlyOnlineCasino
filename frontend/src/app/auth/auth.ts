...
export class Auth {
  ...
  protected readonly registerFormModel: WritableSignal<RegisterFormModel> = signal(
    Auth.REGISTER_DEFAULT,
  );
  ...
  protected async handleLogIn() {
    await submit(this.loginForm, async (form) => {
      ...
      try {
        await firstValueFrom(this.service.logIn(username, password));
  ...
  protected async handleRegister() {
    await submit(this.registerForm, async (form) => {
      ...
      try {
        await firstValueFrom(this.service.register(username, password));
